...
import {useGame, useSoloGame} from "../../../data/SoloGamesData";
...
const TeamSplash = ({teamDetails}) => {
    return (
        <div style={{margin: 40, display: 'flex', flexDirection: 'column',maxWidth: 270, alignItems: 'center'}}>
            <TeamPicture size={'large'} player1Hash={teamDetails.players[0].avatarHash} player2Hash={teamDetails.players[1].avatarHash}/>
            <TeamTooltip teamId={teamDetails.teamWithStats.id}>
                <Typography noWrap style={{maxWidth:'100%',marginTop: 10, overflow: 'hidden', textOverflow: 'ellipsis'}} variant={"h5"}>{teamDetails.teamWithStats.name}</Typography>
            </TeamTooltip>
        </div>
    )
}
...
const TeamStats = ({game, team}) => {
    ...
    return (<div
        ...
        <Typography style={{maxWidth: '90%',textOverflow: 'ellipsis',overflow:'hidden'}} noWrap color={"primary"} variant={"h5"}>{teamDetails.teamWithStats.name}</Typography>
        {nakedLap &&
